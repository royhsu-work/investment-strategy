...
- [ ] **RED** Add tests for a valid Human-created decision comment plus later Human-only `human:approved` event and unchanged comment revision.
...
- [ ] **GREEN** Implement the minimum reusable Human-authority evaluator from explicit comment/event evidence without hidden authorization state or a generic IAM layer.
- [ ] **REFACTOR/VERIFY** Run focused tests plus the full repository regression, type, lint, and format gates.

Trace: proposal items 1–2, 4; spec requirement `Human-required authority is bound to provenance-validated repository Human decisions`; design D1/D3.
...
- [ ] **RED** Add Human-only admission tests proving connector-mediated decision/approval evidence cannot satisfy Human admission.
- [ ] **RED** Add Human-answer/resume tests proving only the intended provenance-bound decision can resolve a current Human boundary.
- [ ] **RED** Add authorization tests for current Human-reserved proceed/stop or other authorization consumers that still rely on actor-only evidence.
...
- [ ] **GREEN** Replace duplicated actor-only Human checks with the shared predicate only at Human-reserved consumers, preserving repository-authorized Explore semantics.
...
Trace: proposal item 5; spec Human-reserved consumer and repository-authorized Explore scenarios; design D4.
...
- [ ] **RED** Add migration tests proving completed historical workflows remain terminal while still-pending Human-reserved evidence consumed after activation must satisfy the stronger predicate.
- [ ] **GREEN** Update shared governance/role/skill wording and any label fixtures needed for the exact two-capability relationship and prospective activation boundary.
