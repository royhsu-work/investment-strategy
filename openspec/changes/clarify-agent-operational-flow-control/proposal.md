...
- Add one bounded premature-close recovery rule for the demonstrated #40 class: when durable evidence proves one closed formal coordination Issue is unfinished, was not closed through the authorized terminal Archive boundary, has no qualifying Human terminal decision, and reopening cannot create multiple-active ambiguity, `Lead / resolve-question` owns deterministic reopen recovery. Reopening preserves the immutable Change and existing nonterminal routing, then requires a fresh repository-wide reconstruction before any normal lifecycle action resumes.
...
- `scheduled-agent-workflow` — operational eligibility, WIP/queue ordering, premature-close recovery, deferred-follow-up admission, pre-activation fallback, and state-coherence rules.
...
- a global blocker label/result taxonomy, priority scoring engine, expedite lane, claim/lease/heartbeat, retry counter, hidden backlog, generic fault-state/recovery machine, or second workflow DAG;
...
Source: #65 decision-complete Explore `issuecomment-5317001684`, including the #40 premature-close incident, #48/#49 multiple-active incident, required-follow-up routing incidents (#49/#63), direct-Propose fallback hypothesis, scheduler auto-pause observation, and active-workflow enumeration failure case; Reviewer correction `issuecomment-5317087714` requires decision-complete premature-close recovery ownership and preconditions.
