...
Trace: Issue #180 current Human invariant → openspec/config.yaml existing authoring owner → Design D1/D4.

- [ ] 1.1 RED — Add focused regression coverage proving the current config has the existing complexity/traceability rules but does not yet state recursive stage decomposition and parent-outcome reconciliation. Verify the failure is the missing predicate.
- [ ] 1.2 GREEN — Add the smallest config rules for N-1-independent stages, parent-outcome preservation, minimum stage evidence, explicit approved reduction/defer, and the remove/reuse/consolidate/existing-owner/add order.
- [ ] 1.3 REFACTOR — Keep the rules at the existing config owner; remove any directly duplicated staged-delivery wording from touched procedural surfaces. Do not copy the project-wide proportionality requirement.
...
Trace: Issue #180 reusable-procedure decision → Design D2/D4.

- [ ] 2.1 RED — Add focused structural tests for the required procedure order, parent/current/remaining reconciliation, explicit reduction/defer distinction, and absence of routing/state/registry authority.
...
Trace: Issue #180 mapped-consumer decision → Design D3/D5.
...
- [ ] 3.4 REFACTOR — Remove any directly adjacent duplicate staged-delivery guidance while preserving current action-specific meaning. Do not refactor unrelated repository prose.
...
Trace: Issue #180 parent-outcome exit criteria → Design D4/D5/D6.
...
If any stage exits with approved parent outcome still incomplete, record the remaining mandatory outcome and required continuation and return the existing MORE_IMPLEMENTATION_REQUIRED result. Only an explicit approved reduction/defer decision may remove that remainder.
