...
- [x] RED: add focused mapped-Skill regressions where an internally consistent target contradicts its referenced Explore result and must fail review, plus a faithful formalization case.
- [x] GREEN: update `openspec-change` to record/dereference the exact durable Explore result and preserve its material boundary.
- [x] GREEN: update `openspec-review` to verify preservation before ordinary bidirectional traceability.
- [x] GREEN: keep direct-to-Propose unchanged and keep the spec-driven semantic adapter outside workflow-handoff ownership.
- [x] REFACTOR: keep mapped Skills action-local and reference shared governance rather than duplicating the full invariant.
- [x] VERIFY: run focused tests, full quality gates, and strict OpenSpec validation before marking the slice complete.
...
- [x] Confirm Proposal / Specs / Design / Tasks are bidirectionally traceable and declare current source Explore #86 issuecomment-5352138330.
- [x] Run strict OpenSpec validation for the exact handoff revision and resolve all findings.
- [x] Do not mark implementation tasks complete during Propose; Executor owns implementation completion evidence after independent OpenSpec PASS.
