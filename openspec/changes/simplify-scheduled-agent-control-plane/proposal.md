...
#138 formalizes the architecture reset from same-Issue Explore `issuecomment-5474475020` on `main@2eb17faa57248ed8218d6ccd47441837268e51e9` plus Human architecture clarifications `issuecomment-5475109024` and `issuecomment-5477274582`. The root cause is duplicated machine workflow ownership across routing labels, prose topology, history parsing, transport and wake orchestration; the latest clarification also proves an execution-boundary deadlock when repository-owned OpenSpec mutation can create revision `R` without a runnable exact-`R` validator.
...
Source for every entry: #138 `issuecomment-5474475020`; Human clarifications `issuecomment-5475109024` and `issuecomment-5477274582`; Change `simplify-scheduled-agent-control-plane`.
