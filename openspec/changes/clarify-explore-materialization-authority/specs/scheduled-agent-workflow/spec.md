...
Authority to materialize one bounded pre-activation Formal Explore MUST follow the qualified source decision rather than the physical GitHub writer. When the interaction layer has established an explicit Human request to open one bounded Formal Explore, that request SHALL authorize materialization of exactly one complete pre-activation tuple consisting of an open coordination Issue, `Change: unset`, and `action:explore-change`. Materialization MUST be considered successful only after fresh observation proves the complete tuple; Issue existence without the requested coherent routing is incomplete. A connector or Agent acting as the physical mutation carrier neither grants nor removes this source-decision authority.

After materialization, ordinary Explore selection SHALL use current repository workflow state and the existing formal-first, WIP=1, finish-first, and deterministic pre-activation ordering predicates without treating Human-versus-connector origin as a routing dimension. Connector-mediated materialization MUST NOT satisfy a later Human-reserved decision predicate. An autonomous Agent recommendation without an explicit Human request or another repository-qualified producer MUST remain advisory and MUST NOT self-authorize materialization.
...
#### Scenario: Explicit Human request materializes one complete bounded Explore

- GIVEN the interaction layer has established an explicit Human request to open one bounded Formal Explore
- AND the connector or Agent is only the physical GitHub mutation carrier
- WHEN the repository materializes that request
- THEN exactly one open coordination Issue is observed with `Change: unset` and `action:explore-change`
- AND physical writer identity neither grants nor removes the qualified source-decision authority
- AND materialization is incomplete if the Issue exists without that coherent routing tuple

#### Scenario: Materialized Explore remains subject to normal queue ordering

- GIVEN a Human-requested bounded Explore has been coherently materialized
- AND another formal workflow currently occupies WIP
- WHEN repository-owned dispatch evaluates work
- THEN the new Explore remains queued under existing formal-first and finish-first semantics
- AND its Human-versus-connector writer origin does not bypass or alter queue ordering

#### Scenario: Autonomous recommendation does not self-authorize materialization

- GIVEN an Agent independently recommends a new Explore
- AND there is no explicit Human request and no existing repository-qualified producer for that work
- WHEN materialization authority is evaluated
- THEN the recommendation remains advisory
- AND no routed Formal Explore is authorized solely from the recommendation or Agent writer identity

#### Scenario: Connector materialization is not later Human-reserved proof

- GIVEN a connector physically materialized a bounded Explore from an explicit Human request
- WHEN that connector-created Issue or event is presented as evidence for a later Human-reserved decision
- THEN existing provenance-bound Human authority still applies
- AND connector activity or writer identity alone is insufficient proof
