...
- [ ] 1.2 GREEN — Vendor the complete pinned upstream subtree to `agents/skills/skill-creator/` without modifying or deleting upstream-origin file content, including all agents/assets/eval-viewer/references/scripts and `LICENSE.txt`.
- [ ] 1.3 GREEN — Add `agents/skills/skill-creator/UPSTREAM.md` (or equivalently bounded provenance metadata) recording upstream repository/path, commit, subtree SHA, upstream inventory/integrity basis, and explicit Added / Deleted / Modified sections. Every difference must state its concrete repository reason; empty categories must say `none`. For the initial baseline, upstream Deleted and Modified are `none`, while repository-authored local additions are listed with reasons.
- [ ] 1.4 REFACTOR — Remove any duplicated manifest/test data that is not needed to deterministically distinguish pinned upstream files from declared Added / Deleted / Modified differences while preserving immutable provenance and reviewability.
...
- [ ] 2.3 GREEN — Remove `agents/skills/skill-maintenance.md` after the material local guidance is preserved and provenance identifies the replacement as a repository-authored Added file with its reason.
