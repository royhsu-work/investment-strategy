...
- Preserve the exact durable Explore result as semantic authority for Propose/Reviewer without using it as queue state.
...
- Provide executable regression coverage at the actual dispatcher/application boundaries.
...
- No OpenAI API/model call for result classification.
...
### Decision 2: Semantic readiness fails the selected action, not selection

`Lead / propose-change` retains the requirement to dereference the exact durable same-Issue Explore `ACTION_RESULT(PROPOSAL_READY)` and validate that its still-applicable scope, constraints, exclusions, selected direction, and feasibility evidence support formalization.

If that evidence is missing, ambiguous, stale, or contradictory, the selected Propose action fails closed and retains the same Issue/routing for legal diagnosis/correction. The dispatcher does not reinterpret the failure as candidate ineligibility and does not fall through to a later Issue.

Reviewer retains the corresponding upstream semantic traceability obligation. This preserves #137's evidence-backed readiness intent while moving the evidence to the consumer that actually needs it.

### Decision 3: Remove Human direct-Propose from normal intake

The normal path becomes:

```text
routed Explore
→ structured PROPOSAL_READY
→ repository-derived Lead / propose-change routing
→ Propose action-local semantic revalidation
→ Change activation
```

The direct-Propose decision reference and special admission/fallback/review exceptions are deleted. This does not change the provenance-bound Human predicate for `HUMAN_DECISION_REQUIRED`, advisory admission, or later Human-reserved scope/risk/architecture decisions.

A coherent Propose tuple written administratively/out-of-band is still current operational routing and therefore selectable. It does not manufacture semantic evidence or Human authority; absent the required Explore baseline, the Propose action fails closed on that same Issue.

### Decision 4: Explore disposition is structured separately from narrative evidence
...
Narrative `result_content` remains available for durable `ACTION_RESULT` explanation and traceability. The machine result code is not parsed back out of that Markdown.
...
### Decision 5: Repository application derives Explore effects
...
### Decision 6: Migration is state-preserving
...
mapped Lead worker
  → semantic Explore/Propose judgment

repository application
  → fresh source reauthorization
  → validate bounded result/effect preconditions
  → derive Explore effect
  → mutate
  → fresh postcondition
  → redispatch
```

This keeps classifier correctness separate from input authority/completeness and keeps semantic evidence downstream of the mapped-action boundary.
...
- A selected Propose with bad semantic evidence remains the selected Propose problem; no queue fallback occurs.
...
3. Propose action/application: missing exact Explore semantic evidence retains/fails the selected Issue and cannot cause fallback.
4. Worker/application: structured `PROPOSAL_READY` derives Propose routing and fresh redispatch sees the tuple directly.
5. Worker/application: inconsistent worker-supplied routing cannot override structured Explore result.
6. All four Explore results preserve governed successor/Human/terminal semantics.
7. Direct-Propose admission paths/helpers disappear while advisory and escalation Human authority remain covered.
8. Existing invalid-routing, incomplete-enumeration, multiple-formal-workflow, closed-debt, stale-state, and cardinality regressions stay green.

Strict OpenSpec validation and independent semantic review remain required on the exact final OpenSpec revision before implementation handoff.