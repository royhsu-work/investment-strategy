...
def _comment(*, id: int, minute: int, decision_ref: str, updated_minute: int | None = None, app: str | None = None, provenance_available: bool = True) -> DecisionComment:
    return DecisionComment(id=id, created_at=_ts(minute), updated_at=_ts(updated_minute if updated_minute is not None else minute), author="royhsu-work", body=f"decision\nHuman-Decision-For: {decision_ref}", provenance_available=provenance_available, performed_via_github_app=app)


def _event(*, id: int, minute: int, app: str | None = None, provenance_available: bool = True) -> LabelEvent:
    return LabelEvent(id=id, created_at=_ts(minute), actor="royhsu-work", label="human:approved", provenance_available=provenance_available, performed_via_github_app=app)


def _approved(expected_ref: str, *, comments: tuple[DecisionComment, ...], events: tuple[LabelEvent, ...], label_present: bool = True) -> bool:
    return is_human_decision_approved(expected_ref=expected_ref, approval_label_present=label_present, comments=comments, label_events=events)


def _raw_issue(*, body: str = "Admission: Lead / explore-change\nChange: unset\n\nInvestigate the issue.", app: object = None, include_provenance: bool = True, actor: str = "royhsu-work", updated_at: str | None = None) -> dict[str, object]:
    raw: dict[str, object] = {"id": 500, "created_at": "2026-08-16T07:00:00Z", "user": {"login": actor}, "body": body}
...
def test_actor_identity_alone_is_not_human_authority() -> None:
    ...
    assert not _approved(decision_ref, comments=(_comment(id=1, minute=1, decision_ref=decision_ref, app="chatgpt"),), events=(_event(id=2, minute=2),))
    assert not _approved(decision_ref, comments=(_comment(id=1, minute=1, decision_ref=decision_ref),), events=(_event(id=2, minute=2, app="chatgpt"),))
    assert not _approved(decision_ref, comments=(_comment(id=1, minute=1, decision_ref=decision_ref, provenance_available=False),), events=(_event(id=2, minute=2),))


def test_valid_human_comment_and_later_human_approval_event_pass() -> None:
    decision_ref = propose_admission_ref(47)
    assert _approved(decision_ref, comments=(_comment(id=10, minute=1, decision_ref=decision_ref),), events=(_event(id=20, minute=2),))
...
def test_event_first_binding_prevents_one_event_from_fanning_out() -> None:
    ...
    comments = (_comment(id=10, minute=1, decision_ref=ref_one), _comment(id=11, minute=2, decision_ref=ref_two))
...
def test_replacement_comment_requires_a_later_approval_event() -> None:
    ...
    assert _approved(decision_ref, comments=(original, replacement), events=(first_event, _event(id=40, minute=4)))
...
def test_post_approval_edit_invalidates_until_later_approval_event() -> None:
    ...
    assert _approved(decision_ref, comments=(edited,), events=(_event(id=20, minute=2), _event(id=40, minute=4)))
...
def test_raw_adapter_preserves_missing_provenance_as_fail_closed_evidence() -> None:
    decision_ref = explore_admission_ref(47)
    raw_comment: dict[str, object] = {"id": 10, "created_at": "2026-08-16T07:01:00Z", "updated_at": "2026-08-16T07:01:00Z", "user": {"login": "royhsu-work"}, "body": f"decision\nHuman-Decision-For: {decision_ref}"}
    raw_event: dict[str, object] = {"id": 20, "created_at": "2026-08-16T07:02:00Z", "actor": {"login": "royhsu-work"}, "event": "labeled", "label": {"name": "human:approved"}, "performed_via_github_app": None}
...
def test_raw_adapter_distinguishes_human_and_app_provenance() -> None:
    ...
    raw_comment: dict[str, object] = {"id": 10, "created_at": "2026-08-16T07:01:00Z", "updated_at": "2026-08-16T07:01:00Z", "user": {"login": "royhsu-work"}, "body": f"decision\nHuman-Decision-For: {decision_ref}", "performed_via_github_app": None}
    raw_event: dict[str, object] = {"id": 20, "created_at": "2026-08-16T07:02:00Z", "actor": {"login": "royhsu-work"}, "event": "labeled", "label": {"name": "human:approved"}, "performed_via_github_app": {"id": 1, "slug": "connector"}}
    comment = decision_comment_from_raw(raw_comment)
    event = label_event_from_raw(raw_event)
...
def test_unlabeled_event_never_establishes_authority() -> None:
    raw_event: dict[str, object] = {"id": 20, "created_at": "2026-08-16T07:02:00Z", "actor": {"login": "royhsu-work"}, "event": "unlabeled", "label": {"name": "human:approved"}, "performed_via_github_app": None}
...
def test_current_human_reserved_boundaries_use_exact_serialized_anchors() -> None:
    ...
    assert decision_ref_for_boundary(HumanDecisionBoundary.EXPLORE_ADMISSION, issue_number=52) == explore_admission_ref(52)
    assert decision_ref_for_boundary(HumanDecisionBoundary.PROPOSE_ADMISSION, issue_number=52) == propose_admission_ref(52)
    assert decision_ref_for_boundary(HumanDecisionBoundary.ADVISORY_ADMISSION, issue_number=52) == advisory_admission_ref(52)
    assert decision_ref_for_boundary(HumanDecisionBoundary.ESCALATION_RESPONSE, escalation_comment_id=5303804185) == escalation_response_ref(5303804185)
...
def test_human_advisory_admission_requires_distinct_intake_capability() -> None:
    ...
    comments = (_comment(id=10, minute=1, decision_ref=decision_ref),)
    events = (_event(id=20, minute=2),)
    assert not is_human_advisory_admission_approved(issue_number=47, intake_approval_label_present=False, human_approval_label_present=True, comments=comments, label_events=events)
    assert is_human_advisory_admission_approved(issue_number=47, intake_approval_label_present=True, human_approval_label_present=True, comments=comments, label_events=events)
...
def test_human_created_formal_explore_requires_exact_raw_creation_contract() -> None:
    ...
    assert is_human_created_explore_admission(creation=creation, current_agent_label="agent:lead", current_action_label="action:explore-change", declaration_history_unambiguous=True)
...
def test_issue_updated_at_is_not_used_as_creation_declaration_history_proxy() -> None:
    ...
    assert is_human_created_explore_admission(creation=creation, current_agent_label="agent:lead", current_action_label="action:explore-change", declaration_history_unambiguous=True)
    assert not is_human_created_explore_admission(creation=creation, current_agent_label="agent:lead", current_action_label="action:explore-change", declaration_history_unambiguous=False)


@pytest.mark.parametrize(("raw", "agent", "action", "history_ok"), [(_raw_issue(app={"id": 1, "slug": "connector"}), "agent:lead", "action:explore-change", True), (_raw_issue(include_provenance=False), "agent:lead", "action:explore-change", True), (_raw_issue(body="Change: unset"), "agent:lead", "action:explore-change", True), (_raw_issue(body="Admission: Lead / explore-change\nAdmission: Lead / explore-change\nChange: unset"), "agent:lead", "action:explore-change", True), (_raw_issue(body="Admission: Lead / propose-change\nChange: unset"), "agent:lead", "action:explore-change", True), (_raw_issue(), "agent:reviewer", "action:explore-change", True), (_raw_issue(), "agent:lead", "action:propose-change", True), (_raw_issue(), "agent:lead", "action:explore-change", False)])
def test_human_created_formal_explore_fails_closed_for_invalid_evidence(raw: dict[str, object], agent: str, action: str, history_ok: bool) -> None:
    creation = issue_creation_from_raw(raw)
    assert not is_human_created_explore_admission(creation=creation, current_agent_label=agent, current_action_label=action, declaration_history_unambiguous=history_ok)


def test_human_created_formal_explore_requires_one_unset_change_declaration() -> None:
    for body in ("Admission: Lead / explore-change\nChange: active-change", "Admission: Lead / explore-change\nChange: unset\nChange: unset", "Admission: Lead / explore-change\nChange: unset\nChange: another-change"):
        assert not is_human_created_explore_admission(creation=issue_creation_from_raw(_raw_issue(body=body)), current_agent_label="agent:lead", current_action_label="action:explore-change", declaration_history_unambiguous=True)
...
def test_human_explore_admission_falls_back_to_existing_general_predicate() -> None:
    ...
    assert is_human_explore_admission_approved(issue_number=issue_number, creation=connector_creation, current_agent_label="agent:lead", current_action_label="action:explore-change", declaration_history_unambiguous=False, approval_label_present=True, comments=(_comment(id=10, minute=1, decision_ref=decision_ref),), label_events=(_event(id=20, minute=2),))
...
def test_creation_shortcut_does_not_mutate_general_decision_semantics() -> None:
    ...
    assert _approved(propose_ref, comments=(_comment(id=10, minute=1, decision_ref=propose_ref),), events=(_event(id=20, minute=2),))
