...
2. Decide whether the finding is accepted, rejected, or already resolved using approved scope and evidence. Explain the decision durably. When that approved specification/scope decision explicitly creates a required deferred follow-up that must still be handled as a separate change, create or reuse one source-linked tracker immediately: preserve the source coordination Issue/Change and exact defer decision/reference, set `Change: unset`, and route it as `agent:lead + action:explore-change` without Human admission. The approved required-deferred decision itself is the bounded repository-authorized Explore authority; do not require a second idle-discovery admission step. Ordinary out-of-scope, non-goal, optional, or merely deferred prose does not create or route a tracker.
