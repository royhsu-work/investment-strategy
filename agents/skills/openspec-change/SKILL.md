...
2. If `Change:` is unset and Propose remains proposal-ready, reconstruct formal active/terminal-pending workflow state and the shared combined pre-activation queue before persisting any Change identity. The procedure must reconstruct active workflow state before persisting an unset Change identity and MUST use the shared executable pre-dispatch complete-cardinality decision from same-invocation authoritative GitHub evidence, not an action-local query or Agent summary.
   ...
   - Immediately before the activation write, obtain a fresh complete provenance-qualified repository snapshot, execute the shared helper, and require its exact decision to authorize this Issue as `Lead / propose-change` with formal cardinality zero and this Issue still the combined pre-activation winner. Only that executable AUTHORIZE decision may permit the write.
   ...
   - Immediately after the write, obtain a newly fresh complete provenance-qualified repository snapshot and execute the helper again. Activation is accepted only when the resulting decision proves exactly one formal active workflow, this Issue, with the expected Change identity and current routing. If a competing activation, multiple-active state, incomplete/indeterminate provenance, helper execution failure, or newer contradictory state is observed, activation is not accepted and the invocation stops stale/fail-closed; do not choose a winner, reuse the pre-write decision, or rewrite another Change/routing tuple.
