...
For workflow-dynamic `propose-change`, both activation boundaries MUST consume the repository-owned executable dispatch precondition in `src/investment_strategy/workflow_dispatch.py` using complete normalized evidence whose authorization-bearing current Issue state, Change identity, routing labels, and enumeration provenance come from authoritative GitHub observations obtained during this invocation. Conversation/history, prior invocation output, cached observations, and historical Issue body/comment routing are context only and MUST NOT fill missing current fields. The immediate pre-write decision must authorize this exact Issue and `Lead / propose-change`; after the Change write, a newly reconstructed executable decision must prove this Issue is the sole formal active workflow with the expected Change and current routing. If the helper cannot execute, current provenance/completeness cannot be established, or the post-write decision is multiple/contradictory/indeterminate, activation is not accepted and no legal successor is inferred. Retain the exact pre-write and post-write decisions for the action-result evidence required by the shared contract.
...
For an Explore-originated `propose-change`, reconstruct the exact durable Explore `ACTION_RESULT` that established `PROPOSAL_READY` for this same coordination Issue. Treat that exact result as the upstream semantic baseline for formalization: proposal/readiness evidence MUST identify it exactly, and Lead MUST preserve every material decided scope, constraint, exclusion, and selected direction that remains applicable. Missing, ambiguous, stale, or contradictory Explore-result provenance fails closed rather than being replaced by Issue prose or conversation memory. Direct-to-Propose has no preceding Explore result and MUST NOT fabricate a synthetic Explore reference.
...
2. If `Change:` is unset and Propose remains proposal-ready, reconstruct formal active/terminal-pending workflow state and the shared combined pre-activation queue before persisting any Change identity. The procedure must reconstruct active workflow state before persisting an unset Change identity and MUST use the shared executable pre-dispatch complete-cardinality decision from same-invocation authoritative GitHub evidence, not an action-local query or Agent summary.
   - If a formal active/terminal-pending workflow exists, keep this Issue queued and perform no activation.
   - If multiple formal active workflows, indeterminate enumeration/provenance, helper execution failure, or contradictory durable identity evidence exist, fail closed.
   ...
   - Immediately before the activation write, re-read durable state and require this Issue to remain the combined pre-activation winner; obtain a fresh complete provenance-qualified repository snapshot, execute the shared helper, and require its exact decision to authorize this Issue as `Lead / propose-change` with formal cardinality zero. Only the first valid activation with an executable `AUTHORIZE` decision may continue.
   ...
   - Immediately re-read durable state after the write by obtaining a newly fresh complete provenance-qualified repository snapshot and executing the helper again. Activation is accepted only when the resulting decision proves exactly one formal active workflow, this Issue, with the expected Change identity and current routing. If a competing activation, multiple-active state, incomplete/indeterminate provenance, helper execution failure, or newer contradictory state is observed, stop as stale/fail-closed; do not choose a winner, reuse the pre-write decision, or rewrite another Change/routing tuple.
