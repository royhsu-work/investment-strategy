...
2. If `Change:` is unset and Propose remains proposal-ready, reconstruct formal active/terminal-pending workflow state and the shared combined pre-activation queue before persisting any Change identity. The procedure must reconstruct active workflow state before persisting an unset Change identity.
   ...
   - If no formal active/terminal-pending workflow exists, consume the complete shared pre-activation candidate-set contract from default-branch `agents/AGENTS.md`: every legally reconstructed open `Lead / explore-change + Change: unset` entry from the approved Explore-origin set plus every Human-admitted `Lead / propose-change + Change: unset` entry participates in the same combined queue. Do not maintain or infer a narrower action-local Explore-origin enumeration.
   - Choose the combined-queue winner by earliest GitHub `created_at`, then lower Issue number. A later proposal-ready direct-Propose Issue MUST NOT activate while an older valid Explore candidate from any approved origin—including a same-Issue direct-Propose fallback preserving its original authority envelope—is the deterministic combined pre-activation winner.
