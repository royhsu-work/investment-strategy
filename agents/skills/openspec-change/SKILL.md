...
The worker may author approved specification artifacts in its local checkout when its action capability permits local writes. It has no durable GitHub write authority over repository state. Any requested Issue/comment/label/routing/Change-identity persistence, tree/commit/ref/PR mutation, Human-escalation write, or other durable repository-state effect is invocation-local output for repository-owned application. Application fresh-reauthorizes the exact source action, checks effect-specific preconditions, applies authorized effects, observes postconditions, and re-dispatches from resulting current state.

The M0 self-hosting boundary has one narrow Git-object ingress exception for these mapped Propose/Resolve actions: after authoring Lead-owned OpenSpec work product, the semantic worker MAY create **unreferenced Git blobs only** and use their returned SHA values as untrusted content-addressed ingress. Blob existence is not repository state, authorization, a completed write, or validation evidence. The worker MUST NOT create a Git tree or commit, update/create a ref, use Contents API to persist the work product, or treat any blob as successful application. Full source/spec/test content MUST NOT be carried through `EFFECT_REQUEST`, `WORK_PRODUCT_REQUEST`, or another Issue-comment result transport merely to persist repository files.

To apply such a work product, the worker uses the correlated check-in transport `WORK_PRODUCT_REQUEST` with the exact current dispatch request/decision IDs, current PR, expected Change, and `Manifest-B64`. The decoded manifest contains exactly `branch`, `base_sha`, `message`, and a non-empty `files` list; each file contains only `path`, `blob_sha`, and current `expected_sha`. Repository application fresh-reauthorizes the exact source action, verifies the current PR/head/base, Change-owned paths, current blob identities and every referenced blob, constructs one exact multi-file Git tree and single commit revision `R`, advances only the exact current branch ref without force, fresh-observes ref/PR/commit/file postconditions, and exposes `R` to the same exact-revision OpenSpec validation boundary. Any stale base/path/blob/current identity fails closed. This M0 carrier does not gain Issue/Action/successor/retry/success authority.

After a cross-role `EFFECT_REQUEST` has durably persisted the source `ACTION_RESULT` and repository application has applied and observed routing to `Reviewer / review-openspec`, the current invocation completes the required application-owned journal through `HANDOFF_COMPLETION_REQUEST`. That request contains only the same dispatch request/decision IDs, exact applied `EFFECT_REQUEST` comment ID, exact durable source `ACTION_RESULT` comment ID, and expected Change. Repository application dereferences those exact sources, derives the cross-role target from the already-applied worker result, fresh-observes target routing, persists canonical `HANDOFF`, and fresh-observes the handoff postcondition. The semantic worker MUST NOT author or substitute the HANDOFF body itself.

For `propose-change`, the activation write is an application-time effect boundary only while `Change: unset`. The immediate pre-write machine decision must authorize this exact Issue as `Lead / propose-change`; after the requested Change-identity write is applied, repository runtime must fresh-reconstruct current GitHub state and execute the classifier again. Activation is accepted only when that post-write decision proves this Issue is the sole formal active workflow with the expected Change identity and current routing. A `propose-change` invocation reached after bounded formal pre-first-review Explore correction already has a non-`unset` immutable Change and MUST NOT replay or replace activation.
...
For `propose-change`, consume the exact machine authorization/evidence envelope supplied for this worker invocation. Candidate-local or partial enumeration is never sufficient evidence, and the worker MUST NOT re-derive an alternative dispatcher decision from Issue prose. The executable decision proves current selection only; it does not establish semantic readiness or replace the action-local Explore baseline.
...
For every `propose-change`, reconstruct exactly one durable same-Issue Explore `ACTION_RESULT` that established the `PROPOSAL_READY` baseline governing this formalization position. Treat that exact result as the upstream semantic baseline and independently verify its material source/evidence chain: each material Explore claim needed for scope, constraints, feasibility, selected direction, or the Human boundary must still be supported by referenced source/evidence, with source fact/evidence distinguishable from Lead interpretation/inference and unresolved questions. Proposal/readiness evidence MUST identify that exact Explore result and material source/evidence chain, and Lead MUST preserve every material decided scope, constraint, exclusion, feasibility conclusion, and selected direction that remains supported and applicable.

Missing, ambiguous, stale, contradictory, unsupported, or materially invalidated baseline/source evidence does not cause dispatcher fallback to another Issue. If the gap remains researchable within the same bounded problem and no new Human-reserved decision is required, Propose returns structured `RESEARCH_REQUIRED`; the worker MUST NOT request the correction routing itself. Before activation, repository-owned application derives the same Issue `Lead / propose-change → Lead / explore-change` correction while preserving `Change: unset` and the Issue's original queue identity. After activation, the same correction is legal only before the first independent `Reviewer / review-openspec` PASS for the Change and preserves the immutable non-`unset` Change identity, existing PR/artifact history, durable evidence, and formal WIP. After the first independent OpenSpec acceptance, material formal correction uses `Lead / resolve-question`. If the gap requires a new Human-reserved requirement, scope/risk acceptance, or architecture decision, use the governed Human boundary.

If routing, change identity, active-workflow identity, review-acceptance history, or required evidence is contradictory, fail closed.
...
1. Confirm current valid `Lead / propose-change` routing from the machine-authorized source and classify the formalization position from authoritative state:
   - **pre-activation Propose**: `Change: unset`; dereference the exact same-Issue durable Explore `ACTION_RESULT(PROPOSAL_READY)` and satisfy the existing activation preconditions below;
   - **formal pre-first-review Propose after Explore correction**: the same Issue is the sole active formal workflow with the same non-`unset` immutable Change, no independent `Reviewer / review-openspec` PASS has yet accepted the Change, and the exact durable formal-correction Explore `ACTION_RESULT(PROPOSAL_READY)` is the current causal semantic baseline. This mode MUST NOT request another Change-identity write or reset the existing PR/artifact history.
   Independently verify the supporting source/evidence for every material baseline claim and feasibility sufficiency needed for formalization; preserve the still-applicable supported material boundary throughout formalization. Current coherent routing alone is not semantic readiness.
   - If baseline/source evidence is missing, ambiguous, stale, contradictory, unsupported, or materially invalidated but the gap remains researchable within the same bounded problem and no new Human-reserved decision is required, return `propose_disposition = RESEARCH_REQUIRED`. Request no `routing-transition` and no Change-identity mutation. Repository application owns the deterministic same-Issue correction to `Lead / explore-change`; pre-activation mode preserves `Change: unset`, while formal pre-first-review mode preserves the same non-`unset` Change and formal history. This MUST NOT cause dispatcher fallback to another Issue.
   - If insufficiency exposes a new Human-reserved requirement, product/scope/risk acceptance, or architecture decision, use the governed Human boundary rather than disguising it as research.
   - If a non-`unset` Change has already received an independent `Reviewer / review-openspec` PASS, do not use backward Explore correction; use the formal `Lead / resolve-question` path for material correction.
...
- researchable formal pre-first-review material source/evidence or feasibility insufficiency, with the same non-`unset` Change and no independent `review-openspec` PASS or new Human-reserved decision → `RESEARCH_REQUIRED`; request no routing/Change-identity mutation. Repository application derives same-Issue `Lead / explore-change` while preserving the existing Change, PR/artifact history, evidence, and WIP.
...
- source no longer machine-authorized or activation/formal position is displaced by current formal/debt/pre-activation state → retain `Lead / propose-change` without mutation noise and let fresh repository dispatch own later selection.
- formal semantic problem after independent OpenSpec acceptance, or a non-feasibility correction that belongs to the approved formal correction loop → use `Lead / resolve-question`.
...
For workflow-dynamic `Lead / propose-change`, the applicable `ACTION_RESULT` renders the exact activation evidence consumed by repository dispatch/application when activation occurs: the immediate pre-write executable decision, expected Change identity, post-write formal-active/terminal-pending Issue identities, post-write completeness, post-write observation provenance, post-write disposition, and whether activation accepted. Formal pre-first-review correction reuses the existing immutable Change and MUST NOT manufacture a second activation record.
...
- Except for the M0 unreferenced-blob ingress above, the model worker does not directly persist durable result/routing/label/repository-state mutations; request only bounded effects for repository-owned fresh reauthorization/application.
