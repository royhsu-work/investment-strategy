...
and keep unrelated files untouched. A required separate follow-up must be tied to one exact durable
source decision and one deduplicated target; optional or deferred prose creates no routing obligation.

When content must cross the worker/application boundary, create only unreferenced Git blobs and return
