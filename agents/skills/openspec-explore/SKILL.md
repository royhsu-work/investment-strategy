...
For a pre-activation Propose fallback, reconstruct the same Human direct-Propose authority envelope that legally admitted the `Change: unset + Lead / propose-change` entry. The fallback creates no new Human authority. For transitional compatibility this is the same admitted authority envelope described by the older contract; Explore remains bounded to it, returns to `Lead / propose-change` after in-scope `PROPOSAL_READY`, and requires no second Human admission. These phrases preserve test/audit continuity during Slice 1 rather than reintroducing a generic Explore admission gate.
...
The older phrase `creation-bound Human Explore admission alternative` is retained temporarily in Slice 1 as historical compatibility vocabulary only. It does not govern ordinary routed Explore execution; the shared governance owns current queue semantics. Slice 2 removes this obsolete Explore-only authority machinery and compatibility text together with the old general provenance-bound Human decision path for Explore, while preserving provenance-bound Human authority for direct-to-Propose and genuine Human-reserved decisions.
