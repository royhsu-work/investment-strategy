...
Before substantive Explore research, consume the shared pre-dispatch classification from `agents/AGENTS.md`. The current complete-cardinality evidence MUST still prove zero formal/terminal work and this Issue MUST still equal the deterministic combined pre-activation winner. A stale, partial, role-local, candidate-local, incomplete, or contradictory reconstruction is not an Explore entry precondition: stop and reconstruct repository-wide state instead of continuing from the earlier selection.

For workflow-dynamic execution, that action-entry authorization MUST be the result of the repository-owned executable dispatch precondition in `src/investment_strategy/workflow_dispatch.py`, evaluated from a complete normalized Issue snapshot whose authorization-bearing current Issue state, Change identity, routing labels, and completeness/provenance are derived from authoritative GitHub observations obtained during this invocation. Conversation/history, prior invocation output, cached observations, and historical Issue body/comment routing are audit/context only and MUST NOT fill missing current fields. If the executable helper cannot be run, or the required current observation provenance/completeness cannot be established, the decision is indeterminate/fail-closed and substantive Explore MUST NOT begin. Retain the exact consumed executable decision for the action-result evidence required by the shared contract rather than re-deriving a second Agent summary.
