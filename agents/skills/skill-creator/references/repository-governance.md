...
Apply progressive disclosure to repository Skills when conditional detail would obscure the common action path. Keep reusable content shared only when there is demonstrated cross-Skill reuse; do not extract a shared resource for hypothetical future reuse or duplicate global governance/role authority into Skills.
