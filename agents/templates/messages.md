...
For `Lead / explore-change`, every material conclusion that affects disposition, scope, constraints, feasibility, selected direction, or a Human-reserved boundary MUST also identify its supporting source/evidence and distinguish source fact/evidence from Lead interpretation/inference and unresolved questions. The durable `Result` records the bounded structured Explore disposition; it does not encode a worker-chosen successor. When that disposition has a governed deterministic effect, the repository-owned application derives and observes that effect under the owning action contract, and the durable message may report the observed result without turning narrative routing prose into control state.
